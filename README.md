...
项目说明：本固件专为 360 V6 路由器定制，基于 GitHub Actions 自动化编译。核心理念：轻量稳定、驱动完整、按需扩展。内置常用核心插件，剩余空间留给用户自由发挥。
...
默认简体中文，支持深色/浅色模式自动切换

🛡️ 拦截

luci-app-adblock

官方轻量化 DNS 过滤，保护全家设备免受广告干扰

🚀 加速

HomeProxy

基于 sing-box 与 nftables，极致的网络中转性能

🖨️ 共享

p910nd

支持 USB 打印机网络共享，让老旧设备焕发新生

🛠️ 工具

TTYD Terminal

网页直接进入 SSH 命令行，无需额外客户端
...
修改工作流CONFIG_TARGET_ROOTFS_PARTSIZE=115
...
重要警告：360 V6 的 NAND 分区由厂商预定义，包含坏块表与定制布局。请勿尝试用网页工具格式化内置 NAND 分区，操作失误可能导致设备无法启动，需拆机用编程器救砖。
...
详细刷机步骤请参考：OpenWrt 官方 IPQ60xx 文档 或社区相关教程。


---
⚠️ 重要注意事项

- 打印机共享：使用 p910nd 共享打印机时，必须在 Windows 打印机属性中关闭「启用双向支持」，否则易出现队列卡死。

- 去广告兼容：adblock 与 mosdns/AdGuard Home 功能重叠且可能冲突，请勿同时启用多个 DNS 过滤插件。

- 内存管理：360 V6 为 512MB RAM。内置插件已优化至轻量级，若后续自行安装 Docker、AdGuard Home 等重型服务，请密切监控 状态 → 内存，避免 OOM 死机。

- 插件安装：稳定版分支确保 opkg install 长期可用。若遇网络问题，可临时切换国内镜像源。

- 硬件差异：360 V6 存在不同批次，若刷机后无法启动，请核对设备闪存类型与分区表。


---
🙏 致谢

- OpenWrt Project – 核心源码与构建系统

- ImmortalWrt – HomeProxy 插件支持

- jerrykuku – Argon 主题

- P3TERX – Actions 编译方案参考

- GitHub Actions – 自动化编译平台
...
---
💡 Star ⭐ 本仓库 以获取更新通知 | 🐛 遇到问题请提交 Issues
