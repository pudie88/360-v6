...
[![OpenWrt](https://img.shields.io/badge/OpenWrt-24.10-00C7B7?logo=openwrt)](https://github.com/openwrt/openwrt)
[![Actions](https://img.shields.io/badge/Actions-AutoBuild-blue?logo=githubactions)](https://github.com/${{ github.repository }}/actions)
...
| ❌ NAND 手动分区 | 用 diskman/fdisk 操作内置闪存 | 🔴 极高（可能变砖） | ❌ 不推荐 |

#### ✅ 方案 1：下次编译时调整（推荐）
只需修改工作流中一行配置，零风险获得 +15MB 可用空间：
```yaml
echo "CONFIG_TARGET_ROOTFS_PARTSIZE=115"  # 原为 100
```
...
#    需谨慎操作，建议参考官方文档
```
> 💡 64GB U 盘仅需 ¥30，空间「无限」扩展，且拔掉即恢复原厂状态，安全灵活。

> ⚠️ **重要警告**：360 V6 的 NAND 分区由厂商预定义，包含坏块表与定制布局。**请勿尝试用网页工具格式化内置 NAND 分区**，操作失误可能导致设备无法启动，需拆机用编程器救砖。
...
> 📖 详细刷机步骤请参考：[OpenWrt 官方 IPQ60xx 文档](https://openwrt.org/toh/hwdata/qihoo/qihoo_360_v6) 或社区相关教程。

---

## ⚠️ 重要注意事项

1. **打印机共享**：使用 `p910nd` 共享 Canon MF4400 等打印机时，**必须在 Windows 打印机属性中关闭「启用双向支持」**，否则易出现队列卡死。
2. **去广告兼容**：`adblock` 与 `mosdns`/`AdGuard Home` 功能重叠且可能冲突，**请勿同时启用多个 DNS 过滤插件**。
3. **内存管理**：360 V6 为 512MB RAM。内置插件已优化至轻量级，若后续自行安装 `Docker`、`AdGuard Home` 等重型服务，请密切监控 `状态 → 内存`，避免 OOM 死机。
4. **插件安装**：稳定版分支确保 `opkg install` 长期可用。若遇网络问题，可临时切换国内镜像源。
5. **硬件差异**：360 V6 存在不同批次，若刷机后无法启动，请核对设备闪存类型与分区表。
...
- [GitHub Actions](https://github.com/features/actions) – 自动化编译平台

---

## 📜 免责声明

本项目仅供学习与研究使用，编译的固件不包含任何商业授权或担保。  
